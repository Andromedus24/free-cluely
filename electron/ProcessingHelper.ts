...
export class ProcessingHelper {
  ...
  constructor(appState: AppState) {
    ...
    // Use OLLAMA_HOST (standardized), fallback to deprecated OLLAMA_URL for backwards compatibility
    const ollamaHost = process.env.OLLAMA_HOST || process.env.OLLAMA_URL || "http://localhost:11434"
    
    // Log deprecation warning if OLLAMA_URL is used
    if (process.env.OLLAMA_URL && !process.env.OLLAMA_HOST) {
      console.warn("[ProcessingHelper] OLLAMA_URL is deprecated, please use OLLAMA_HOST instead")
    }
    
    if (useOllama) {
      console.log("[ProcessingHelper] Initializing with Ollama")
      this.llmHelper = new LLMHelper(undefined, true, ollamaModel, ollamaHost)
    } else {
